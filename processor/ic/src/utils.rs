...
use verity_dp_ic::verify::types::{ADCResponse, CurrencyPair, Token};
...
pub async fn get_token_price(token: &Token, notary_pubkey: &String) -> anyhow::Result<f64> {
    ...
    let stringified_proofs: Vec<String> = proof_types.iter().map(|pt: &verity_dp_ic::verify::types::ProofTypes| pt.to_string()).collect();
...
pub async fn get_currency_pair_price(currency_pair: &CurrencyPair, notary_pubkey: &String) -> anyhow::Result<f64> {
    // get base price
